...
#Using variables selected for LDA
test_errors = rep(0, length(variable_sets)*5)


set.seed(10)
for(i in variable_sets){ #using selected variable sets
  var = mammal_lda_variables[[i]]
  
    for (ii in 1:5) {
  mammal_boost = gbm(as.factor(redlistCategory) ~ .,
                      data=mammal_train_df[,var],n.trees=1000,distribution = "multinomial",
                     interaction.depth=4,shrinkage = c(0.1,0.01,0.001)[ii])
...
mammal_lda_variables[[
  variable_sets[ceiling(which.min(test_errors)/5)]
  ]]
#shrinkage
c(0.1,0.01,0.001)[which.min(test_errors)%%5]
...
for(i in 1:length(mammal_lda_variables)){
  ...
  mammal_svm=svm(as.factor(redlistCategory)∼., 
                  data=mammal_train_df[,var],kernel="polynomial", gamma=1,cost=1)
