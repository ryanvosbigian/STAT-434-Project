...
The best model to predict the redlist status of a mammal species based on its Elton traits was a Logistic Regression. However, logistic regression only determines whether the species is least concern or not, which is less desirable than determining status. 
...
table(log_pred, mammals_log_test2_df$redlistCategory)
```
...
table(lda_pred$class, mammal_test2_df$redlistCategory)
...
table(qda_pred$class,mammal_test2_df$redlistCategory)
...
table(mammal_knn, mammal_knn_test2_df$redlistCategory)
```
...
mean(bag_pred != mammal_test2_df$redlistCategory)
table(bag_pred, mammal_test2_df$redlistCategory)
