...
avibase_ioc_df$Scientific.name = avibase_ioc_df$'ï..Scientific.name'
...
```{r mammal k value selection, eval=FALSE}
...
```{r bird prediction and error rate, eval=FALSE}
...
```{r bird k value selection, eval=FALSE}
...
mammal_svm=svm(as.factor(redlistCategory) ~., 
                  data=mammal_train_df[,var],kernel="radial", gamma=0.5,cost=1)
pred_svm = predict(mammal_svm,newdata = mammal_test_df[,var])
#mean(pred_svm != mammal_df$redlistCategory)
0.3361117
#table(pred_svm,mammal_df$redlistCategory)
...
However, logistic regression only determines whether the species is least concern or not, which is less desirable than determining status. 
After logistic regression, K-Nearest Neighbors provides the best error rate. Thus, we will use KNN to predict the unknown status of species.
...
Neural Networks, Support Vector Machines, and K-Nearest-Neighbors were all equivalent in predicting the redlist status of a bird species.  However, all expect for QDA and bagged trees almost everything classified as least concern. Thus, we will use QDA, which of the models that do not classify everything as least concern has the lowest test error.
...
Based on the above model results we will use KNN for predicting mammals and QDA for predicting birds.

```{r predict mammals}

data_deficient_mammals_df


mammal_knn = knn(mammal_df[,c(7, 9, 10, 11, 12, 13, 19, 20, 21)],
                 data_deficient_mammals_df[,c(7, 9, 10, 11, 12, 13, 19, 20, 21)],
                 mammal_df[,38], k=14)


data_deficient_mammals_df$predredlistCategory = mammal_knn

table(mammal_knn)

#write.csv(data_deficient_mammals_df,"best_model_predictions_undeterminedmammals.csv")

data_deficient_mammals_df$scientificName[qda_pred$class=="Endangered"]

```


```{r predict birds}
data_deficient_birds_df
bird_qda = qda(redlistCategory ~ .,
                   data = bird_df[,bird_qda_variables[[2]]])
qda_pred = predict(bird_qda,newdata=data_deficient_birds_df[,bird_qda_variables[[2]]])

data_deficient_birds_df$predredlistCategory = qda_pred$class

table(qda_pred$class)
#write.csv(data_deficient_mammals_df,"best_model_predictions_undeterminedbirds.csv")

data_deficient_birds_df$scientificName[qda_pred$class=="Endangered"]
```


# Conclusions

We used PCA to transform the diet and foraging variables, and included principle components in some of the subsets of variables we tested for each model. We used the following methods:
...
We found that K-Nearest Neighbor had the best predictions based on test error for mammals. For birds, we found that many methods had the same test error but many were attributing all species as least concern. Thus, we used QDA to predict for birds, because of the models that did not attribute all birds as least concern it had the lowest test error rate.

Overall, we found that the models tended to overpredict the status least concern. The final prediction for mammals had only 6 of the 574 species as anything other than least concern, and the final predictions for birds had all 42 species as least concern. 
